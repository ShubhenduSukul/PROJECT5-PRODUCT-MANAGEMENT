...
const addItems = async function(req,res){ // INPUT => {id:productId}
    ...
    // validation of product 
        ...
        if(cartId){
            if(!ObjectId.isValid(cartId)) return res.status(400).send({status:false,msg:'enter a valid cartId in body'})
            var cart = await cartModel.findOne({_id:cartId}) // product Object

            if(!cart) return res.status(404).send({status:false,msg:'no cart found for given cartId'})
        }
        ...
        if(cart){ // if cart is already there
            if(cart.userId.toString() !== req.params.userId) return res.status(403).send({status:false,msg:"cart doesnot belongs to you"})
...
const removeItems = async function(req,res){
    try{
        ...
        let {productId, removeProduct,cartId} = req.body;
        ...
        if(cart.userId.toString() !== req.userId) return res.status(403).send({status:false,msg:"cart doesnot belongs to you"})
        ...
        if(index == -1) return res.status(404).send({status:false,msg:"given product is not found in the cart"})
